...
#test for the "get courses list" endpoint
...
# test for the "Get single course Info" endpoint
def test_get_course_by_id_exists():
    response = client.get("/courses/64f9eb6c87a3127cfa0ef103")
    assert response.status_code == 200
    course = response.json()
    #get the course from the database
    course_db = db.courses.find_one({'_id': ObjectId('64f9eb6c87a3127cfa0ef103')})
    #get the name of the course from the database
    name_db = course_db['name']
    #get the name of the course form the response
...
# test for "Get course Chapter Info" endpoint
def test_get_chapter_info():
    response = client.get("/courses/64f9eb6c87a3127cfa0ef103/1")
    assert response.status_code == 200
...
def test_get_chapter_info_not_exists():
    response = client.get("/courses/64f9eb6c87a3127cfa0ef103/990")
    assert response.status_code == 404
    assert response.json() == {'detail': 'Chapter not found'}
    
# test for "Post Course Rating" endpoint
def test_rate_chapter():
    course_id = "64f9eb6c87a3127cfa0ef103"
    chapter_id = "1"
    rating = 1
